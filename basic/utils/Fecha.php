...
abstract class Fecha{
	...
	*/
	...
	/*
	 Obtiene la diferencia en meses entre dos Fecha recibidas.
	*/
	public static function getDifMesFecha( $fchDesde, $fchHasta ){

		$fchDesde = Fecha::usuarioToBD($fchDesde,1);
		$fchHasta = Fecha::usuarioToBD($fchHasta,1);

		$sql = "select (date_part('year', age) * 12) + date_part('month', age)
 				from (select age($fchHasta::date, $fchDesde::date)) foodate";

		$cant_meses = intval( Yii::$app->db->createCommand($sql)->queryScalar() );
	...
	*/
	...
	//---------------------MANEJO DE FECHAS EN SEGUNDOS---------------------//

	/**
     * Función que se utiliza para convertir una fecha ingresada en formato
     * dd/mm/yyyy en la cantidad de segundos transcurridos entre esa fecha y
     * el 01/01/1970
     */
	public static function fechaASegundos( $fecha = '' ){

		return $fecha != null ? strtotime( str_replace('/', '-', $fecha ) ) : null;

	}

	/**
     * Función que se utiliza para convertir una fecha ingresada en segundos
	 * al formato yyyy/mm/dd utilizado por la base de datos.
     */
	public static function segundosAFecha( $fecha = '' ){
